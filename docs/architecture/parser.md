...
1行ブロックは、ブロック内が既知コマンドまたは `defsub` 呼び出しの場合だけ命令化されます。

```aria
if %flag == 1 { goto *opened }
```

解釈できない1行ブロックは、未解決ジャンプを生成せず構文エラーとして報告します。

```aria
if %flag == 1 { %flag + 1 } ; error
```

`while` は `wend` で閉じる必要があります。閉じ忘れは未解決ラベルではなく構文エラーになります。
