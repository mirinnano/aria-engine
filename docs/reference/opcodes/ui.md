...
テキストボックスの **垂直方向の配置** を切り替えます。ADV モードでテキストを画面中央・上部に配置して読みやすくしたり、好みのレイアウトを選んだりできます。
...
| 引数 | 型 | 説明 |
|------|-----|------|
| `mode` | 文字列 | `top` / `middle` / `bottom`（大文字小文字無視、既定値 = `bottom`） |

| 値 | 配置 | 説明 |
|----|------|------|
| `top` | 上部 | テキストボックス上端をウィンドウ上端から 24px に配置 |
| `middle` | 中央 | テキストボックスを画面中央に配置 |
| `bottom` | 下部（既定） | 従来の ADV モードと同じ画面下端配置 |

**例:**
```aria
; ADV モードでテキストを画面中央に配置
...
**注意:**
- 不明な値を指定した場合は現在の設定を維持します（フォールバック）。
- テーマ初期化時（`Apply*Theme()` 実行時）は自動的に `bottom` にリセットされます。
- `textbox x, y, ...` で明示的に Y 座標を指定した場合はそちらが優先されますが、`top` / `middle` の相対位置指定は以降のテキスト表示で再計算されます。
- 既定値は `Bottom` なので、本機能を導入する前のスクリプトは挙動が変わらず動作します（後方互換性確保）。
