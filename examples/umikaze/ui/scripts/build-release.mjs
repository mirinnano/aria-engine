...
if (!['desktop', 'web'].includes(mode)) {
  console.error("usage: npm run release:desktop | npm run release:web");
  process.exit(2);
}
...
if (mode === "web") {
  ...
  const releaseDir = resolve(repository, "dist/releases/web");
  const cargo = process.env.CARGO || (process.platform === "win32" ? "cargo.exe" : "cargo");
  run(cargo, [
    "run", "--release", "-p", "aria-cli", "--", "package", resolve(gameRoot, "dist/web"),
...
run("npm", ["run", "tauri", "--", "build", "--bundles", bundles], { env });
